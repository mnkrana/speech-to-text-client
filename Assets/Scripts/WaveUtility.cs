...
public class WavUtility
{
        ...
        string dirname = "recordings")
    {
        ...
        if (saveAsFile)
        {
            // filepath = Application.persistentDataPath, dirname, "wav");
            // filepath = System.IO.Path.Combine(GameController.pathToAudioFile, "Audio.wav");
            filepath = System.IO.Path.Combine(Application.persistentDataPath, "Audio.wav");
    ...
    private static byte[] ConvertAudioClipDataToInt16ByteArray(float[] data)
    {
        ...
        Debug.AssertFormat(data.Length * x == bytes.Length, "Unexpected float[] to Int16 to byte[] size: {0} == {1}",
            data.Length * x, bytes.Length);
